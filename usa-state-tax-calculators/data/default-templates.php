...
<div class="sec">
    ...
    <div class="field">
        <label class="lbl">TAX YEAR <span class="ust-tooltip">ℹ️<span class="ust-tooltiptext">2026 uses the latest IRS inflation-adjusted brackets, standard deduction, and Social Security wage base.</span></span></label>
        <div class="sel-wrap">
...
function calculateFederalTax() {
    ...
    const fedYearSel = document.getElementById("fed-year");
    const fyKey = fedYearSel ? fedYearSel.value : "2025";
    const fy = federalYears[fyKey] || federalYears["2025"];
    const stdDeduct = fy.standard_deduction[status];
    ...
    let prevLimit = 0;
    const activeBrackets = fy.brackets[status];
    let marginalRate = 0;
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    const yearSel = document.getElementById("fed-year");
    if (yearSel) {
        yearSel.addEventListener("change", function() {
            const panel = document.getElementById("results-panel");
            if (panel && panel.style.display !== "none") {
...
<div class="sec">
    ...
    <div class="field">
        <label class="lbl">PRIOR-YEAR TOTAL TAX ($) <span class="ust-tooltip">ℹ️<span class="ust-tooltiptext">Last year total tax. Used for the IRS safe-harbor rule (110% of prior year). Leave 0 to use 90% of this year.</span></span></label>
        <div class="inp-wrap">
            <input type="number" class="inp" id="est-prior" value="0">
            <span class="inp-suffix">$</span>
        </div>
        <div class="hint">Optional - enables the 110%-of-last-year safe-harbor target</div>
    </div>
...
<div class="sec" id="results-panel" style="display:none;">
    ...
    <div style="font-size:12px; line-height: 1.6;">
        <div style="display:flex; justify-content:space-between; border-bottom: 1px solid var(--bdr); padding: 6px 0;">
            <span>Est. QBI Deduction (20% Section 199A):</span>
            <strong id="res-est-qbi" style="color:var(--succ);">$0</strong>
        </div>
        <div style="display:flex; justify-content:space-between; border-bottom: 1px solid var(--bdr); padding: 6px 0;">
    ...
    <div class="usc-alert-box blue" id="est-safe-harbor-box" style="margin-top: 15px;">🛡️ Safe-harbor target appears here after you calculate.</div>
...
function calculateEstimatedTax() {
    ...
    const status = (document.getElementById("est-status") || {}).value || "single";
    const yearSel = document.getElementById("est-year");
    const yr = yearSel ? yearSel.value : "2025";
    const fy = federalYears[yr] || federalYears["2025"];
    const priorYearTax = parseFloat((document.getElementById("est-prior") || {}).value) || 0;

    // Self-employment tax (15.3% on 92.35% of net profit), SS capped by year
    const seTaxable = profit * 0.9235;
    const ssPortion = Math.min(seTaxable, fy.ss_wage_base) * 0.124;
    ...
    // QBI (Section 199A): 20% of business income after the 1/2 SE-tax deduction
    const qbiBase = Math.max(0, profit - seDeduction);
    const qbi = qbiBase * 0.20;

    // Year/status-aware standard deduction; QBI reduces taxable income
    const stdDeduct = fy.standard_deduction[status];
    const taxableIncome = Math.max(0, profit - seDeduction - qbi - stdDeduct);

    const brackets = fy.brackets[status];
    let incomeTax = 0;
    ...
    // IRS safe-harbor: the lesser of 90% of current-year tax or 110% of prior-year tax
    const sh90 = totalAnnual * 0.90;
    const safeHarbor = priorYearTax > 0 ? Math.min(sh90, priorYearTax * 1.10) : sh90;
    ...
    const qbiEl = document.getElementById("res-est-qbi");
    if (qbiEl) qbiEl.innerText = "$" + Math.round(qbi).toLocaleString();

    const shBox = document.getElementById("est-safe-harbor-box");
    if (shBox) {
        shBox.innerHTML = "🛡️ <strong>Safe-Harbor Target:</strong> Pay at least <strong>$" + Math.round(safeHarbor).toLocaleString() + "</strong> for the year (about <strong>$" + Math.round(safeHarbor / 4).toLocaleString() + "</strong> per quarter) to avoid an IRS underpayment penalty.";
    }
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    ["est-year", "est-status"].forEach(function(id) {
        const el = document.getElementById(id);
        if (el) el.addEventListener("change", function() {
            const panel = document.getElementById("results-panel");
            if (panel && panel.style.display !== "none") calculateEstimatedTax();
        });
    });
});';
            break;
