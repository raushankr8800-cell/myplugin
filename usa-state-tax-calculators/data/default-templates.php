...
const federalRules = ' . json_encode($tax_db['federal']) . ';
...
<div class="sec">
    ...
    <div class="field">
        ...
        <div class="hint">Adjust gross income by subtracting standard deduction ($15,000 Single)</div>
    </div>
    <div class="field">
...
function calculateCapitalGains() {
    ...
    const otherIncome = parseFloat(document.getElementById("cg-income").value) || 0;

    const gain = Math.max(0, sale - basis);

    // 1. Calculate Short Term Rate
    let shortRate = 0;
    if (status === "married") {
        if (otherIncome <= 23850) shortRate = 0.10;
        else if (otherIncome <= 96950) shortRate = 0.12;
        else if (otherIncome <= 206700) shortRate = 0.22;
        else if (otherIncome <= 394600) shortRate = 0.24;
        else if (otherIncome <= 501050) shortRate = 0.32;
        else shortRate = 0.35;
    } else {
        if (otherIncome <= 11925) shortRate = 0.10;
        else if (otherIncome <= 48475) shortRate = 0.12;
        else if (otherIncome <= 103350) shortRate = 0.22;
        else if (otherIncome <= 197300) shortRate = 0.24;
        else if (otherIncome <= 250525) shortRate = 0.32;
        else shortRate = 0.35;
    }

    // 2. Calculate Long Term Rate
    let longRate = 0;
    if (status === "married") {
        if (otherIncome <= 94050) longRate = 0.00;
        else if (otherIncome <= 583750) longRate = 0.15;
        else longRate = 0.20;
    } else {
        if (otherIncome <= 47025) longRate = 0.00;
        else if (otherIncome <= 518900) longRate = 0.15;
        else longRate = 0.20;
    }

    const shortTax = gain * shortRate;
    const shortPayout = gain - shortTax;

    const longTax = gain * longRate;
    ...
    document.getElementById("res-cg-rate-long").innerText = (longRate * 100) + "%";
    ...
    document.getElementById("res-cg-rate-short").innerText = (shortRate * 100) + "%";

    const boxEl = document.getElementById("cg-savings-box");
    if (savings > 0) {
        boxEl.style.display = "block";
        boxEl.innerHTML = `🎉 <strong>Holding Period Savings:</strong> You save approximately <strong>$${Math.round(savings).toLocaleString()}</strong> in taxes by holding this asset for more than 1 year (Long-term) instead of selling short-term!`;
    } else {
